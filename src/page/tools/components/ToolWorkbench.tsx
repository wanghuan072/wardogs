...
import { calculateBudget, type ToolItem } from "@/lib/tools/calculations";
...
function ListToolbar({ value, onChange, resultCount, placeholder }: { value: string; onChange: (value: string) => void; resultCount: number; placeholder: string }) {
  return <div className={styles.listToolbar}><label><Search aria-hidden="true" /><input value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} aria-label={placeholder} /></label><span>{resultCount} items</span></div>;
}

export function ToolWorkbench({ tool, items }: { tool: string; items: ToolItem[]; query: Query }) {
...
function BudgetPlanner({ items }: { items: ToolItem[] }) {
  const purchasable = useMemo(() => items.filter((item) => ["weapon", "ammo", "attachment", "armor", "medical", "utility", "throwable", "deployable", "vehicle"].includes(item.kind)).sort((a, b) => a.name.localeCompare(b.name)), [items]);
  ...
  const [query, setQuery] = useState("");
  const groupMatch = (item: ToolItem) => group === "all" || group === item.kind || (group === "gear" && ["armor", "medical", "utility", "throwable", "deployable"].includes(item.kind));
  ...
  return <div className={styles.budgetWorkbench}>
    <section className={styles.inventoryPanel}><div className={styles.panelHead}><span>01 / choose items</span><h2>Build a price list</h2></div><label className={styles.cashField}><span>Current cash</span><input type="number" min={0} step={100} value={cash} onChange={(event) => setCash(Math.max(0, Number(event.target.value)))} /></label><div className={styles.filterChips} aria-label="Item groups">{budgetGroups.map(([value, label]) => <button type="button" className={group === value ? styles.chipActive : ""} onClick={() => setGroup(value)} key={value}>{label}</button>)}</div><ListToolbar value={query} onChange={setQuery} resultCount={visibleItems.length} placeholder="Search items you can buy" /><RecordList items={visibleItems} selected={selected} onToggle={toggle} label="Items you can buy" /><p className={styles.formNote}><Info aria-hidden="true" /> {selected.length}/6 selected. $10,000 is the official starting balance. Item prices can change during Early Access.</p></section>
